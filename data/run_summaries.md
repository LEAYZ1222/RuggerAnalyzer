...
## Run R-20260807-2036 — 2026-08-07 20:36 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 150 wallets au-dessus du seuil de score 90 (deux premières tentatives différées par l'analyste, 3e réussie). Après filtrage des 149 déjà connus, 1 seul candidat neuf :
  - `6oA8nS...T21uMV` (score 95) : disqualifié immédiatement en gate B1 pour spray (créateur de 17 tokens sur son propre wallet).
- Aucun candidat n'a atteint la gate B2 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.

### Bilan
- Fichiers mis à jour : `wallets.csv` (+1 ligne, Écartée, commit `cf1ed1e`), `run_log.csv`, `run_summaries.md` — sur `main`. `patterns.csv` inchangé (pas de commit sur ce fichier).
- Budget restant pour le prochain run : 3 ultras / 24 heavies (fenêtre 15 min, rien consommé).
- Prochaine action humaine suggérée : rien à valider ce run (aucun pattern Validé, aucune config tracker en attente).

## Run R-20260807-2123 — 2026-08-07 21:23 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- Outil principal de découverte de wallets fraîchement financés indisponible ce run (3 tentatives : paramètres par défaut, min_score seul, sans aucun paramètre) — même erreur technique que sur plusieurs runs précédents.
- Bascule sur la voie de secours (intel créateurs récents, fenêtre élargie aux single-launch) : 50 créateurs récents passés en revue, triés par activité la plus fraîche.
  - Meilleurs profils single-launch financés par un exchange labellisé (Binance, KuCoin) : scores 26 à 64 — tous loin sous le seuil de 90.
  - Un funder KuCoin déjà connu (lié à P-003/P-004) est désormais associé à un créateur de 4 tokens sur ce lot — spray, rien d'actionnable.
  - Le funder CEX déjà repéré 3 fois sur des near-miss spray (`iGdFcQoyR2M...NSdwu`) réapparaît, toujours associé à du spray (599 tokens sur ce wallet).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260807-2225 — 2026-08-07 22:25 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 50 au-dessus du seuil de score 90. Après filtrage des 47 déjà connus, 3 candidats neufs :
  - `H1ppjpk...C7EkQ7a` (score 95) : disqualifié immédiatement en gate B1 pour spray (créateur de 8 tokens sur son propre wallet).
  - `E94dd6p...cKN6brSAm` (score 95) : disqualifié immédiatement en gate B1 pour spray (créateur de 7 tokens sur son propre wallet).
  - `ChumSMh...CSLUCCAFm` (score 95) : disqualifié immédiatement en gate B1 pour spray (créateur de 2 tokens sur son propre wallet).
- Aucun candidat n'a atteint la gate B2 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.

### Bilan
- Fichiers mis à jour : `wallets.csv` (+3 lignes, toutes Écartées), `run_log.csv`, `run_summaries.md` — sur `main`. `patterns.csv` inchangé (pas de commit sur ce fichier).
- Budget restant pour le prochain run : 3 ultras / 24 heavies (fenêtre 15 min, rien consommé).
- Prochaine action humaine suggérée : rien à valider ce run (aucun pattern Validé, aucune config tracker en attente).

## Run R-20260807-2323 — 2026-08-07 23:23 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 148 au-dessus du seuil de score 90. Croisement complet avec les 188 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents.
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-0122 — 2026-08-08 01:22 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 148 au-dessus du seuil de score 90. Croisement complet avec les 188 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-0226 — 2026-08-08 02:26 UTC
...
## Run R-20260808-0322 — 2026-08-08 03:22 UTC

### Refresh
- 0 tracker actif — rien à recalculer côté surveillance
- Aucune activité sur le feed (aucun rug/trade tracké)

### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 149 au-dessus du seuil de score 90
- 1 seul candidat neuf hors base (`6dhT...VPEB4`, score 95) :
  - Spray détecté : créateur sur 2 tokens depuis la même adresse
...
- Le reste des 149 wallets scorés étaient déjà connus (découverts entre le 05 et le 07/08, exclus par anti-redig <7j)

### Bilan
- Fichiers mis à jour : `wallets.csv` (commit b19f280 sur `main`), `run_log.csv`, `run_summaries.md`
- Budget restant pour le prochain run : 3 ultras / 24 heavies (fenêtre 15 min, rien consommé)
- Prochaine action humaine suggérée : rien à valider ce run (aucun pattern Validé, aucune config tracker en attente)

## Run R-20260808-0428 — 2026-08-08 04:28 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 80 au-dessus du seuil de score 90. Croisement complet avec les 189 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-0625 — 2026-08-08 06:25 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 419 wallets scannés (fenêtre 14j), 30 au-dessus du seuil de score 90. Croisement complet avec les 190 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-0722 — 2026-08-08 07:22 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 149 au-dessus du seuil de score 90. Croisement complet avec les 190 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-0924 — 2026-08-08 09:24 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 30 au-dessus du seuil de score 90. Croisement complet avec les 190 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j) ou disqualifié spray.
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-1023 — 2026-08-08 10:23 UTC
...
## Run R-20260808-1128 — 2026-08-08 11:28 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 419 wallets scannés (fenêtre 14j), 40 au-dessus du seuil de score 90. Croisement complet avec les 190 wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-1225 — 2026-08-08 12:25 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 419 wallets scannés (fenêtre 14j), 30 au-dessus du seuil de score 90. Croisement complet avec les wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j).
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-1325 — 2026-08-08 13:25 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 149 au-dessus du seuil de score 90. Croisement complet avec les wallets déjà connus : **0 candidat neuf** — le lot entier a déjà été vu et traité lors des runs précédents (anti-redig <7j) ou disqualifié spray.
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
## Run R-20260808-1522 — 2026-08-08 15:22 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 148 au-dessus du seuil de score 90. Sur ce lot, 23 wallets sans spray (une seule création chacun). Croisement complet avec les wallets déjà connus : **les 23 sont déjà présents dans wallets.csv**, tous déjà tranchés Écarté (pas de CEX, funder générique, ou pattern non reproductible constaté sur des runs précédents) — **0 candidat neuf**.
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
- Budget restant pour le prochain run : 3 ultras / 24 heavies (fenêtre 15 min, rien consommé).
- Prochaine action humaine suggérée : rien à valider ce run (aucun pattern Validé, aucune config tracker en attente).
## Run R-20260808-1622 — 2026-08-08 16:22 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 148 au-dessus du seuil de score 90. Sur ce lot, 23 wallets sans spray (une seule création chacun). Croisement complet avec les wallets déjà connus : **les 23 sont déjà présents dans wallets.csv**, tous déjà tranchés Écarté — **0 candidat neuf**.
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.
...
- Budget restant pour le prochain run : 3 ultras / 24 heavies (fenêtre 15 min, rien consommé).
- Prochaine action humaine suggérée : rien à valider ce run (aucun pattern Validé, aucune config tracker en attente).
## Run R-20260808-1823 — 2026-08-08 18:23 UTC
...
### Découverte — budget : 3 ultras alloués, 0 consommé, 0 heavy
- 1035 wallets scannés (fenêtre 14j), 148 au-dessus du seuil de score 90. Sur ce lot, 24 wallets sans spray (une seule création chacun). Croisement complet avec les wallets déjà connus : **les 24 sont déjà présents dans wallets.csv**, tous déjà tranchés Écarté — **0 candidat neuf**.
- Aucun candidat n'a atteint la gate B1 → **0 ultra ni heavy dépensé ce run**, budget plein pour le prochain.

### Bilan — aucun changement, pas de commit sur `patterns.csv` / `wallets.csv`
- Fichiers mis à jour : `run_log.csv`, `run_summaries.md` (commit unique sur `main`).
- Budget restant pour le prochain run : 3 ultras / 24 heavies (fenêtre 15 min, rien consommé).
- Prochaine action humaine suggérée : rien à valider ce run (aucun pattern Validé, aucune config tracker en attente).
